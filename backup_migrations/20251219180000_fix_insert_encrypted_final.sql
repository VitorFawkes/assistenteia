...
DROP FUNCTION IF EXISTS insert_message_encrypted(uuid, text, text, text, text, text, text, text, text, boolean, boolean, text, timestamptz, text, text, text, text, text, integer, text);
DROP FUNCTION IF EXISTS insert_message_encrypted(uuid, text, text, text, text, text, text, text, text, boolean, boolean, text, timestamptz, text, text, text, text, text, integer, text, text);

-- Re-create the correct version matching the webhook call
-- The webhook calls with: p_user_id, p_role, p_content, p_encryption_key, p_media_url, p_media_type, p_sender_number, p_sender_name, p_group_name, p_is_group, p_is_from_me, p_wa_message_id, p_message_timestamp, p_quoted_message_id, p_quoted_content, p_file_path, p_file_name, p_mime_type, p_file_size, p_conversation_id
-- It does NOT pass p_status, so we can either omit it or keep it with a default. I will include it with a default to be safe and future-proof.
